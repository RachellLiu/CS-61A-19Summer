def make_fib():
    ...
    count = 0
    def fib():
        nonlocal last_fib, current_fib, count 
        if count == 0:
            count += 1
            return 0
        last_fib, current_fib = current_fib, last_fib + current_fib
        return last_fib
    return fib
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, attempts):
        nonlocal balance, pwlist
        if len(pwlist) >= 3:
            return f'Your account is locked. Attempts: {pwlist}'
        if attempts == password:
            if amount > balance:
                return "Insufficient funds"
            balance = balance - amount
            return balance
        pwlist.append(attempts)
...
class VendingMachine:
    ...
    def deposit(self, depo):
        if self.stock == 0:
            return f"Machine is out of stock. Here is your ${depo}."
        self.balance += depo
