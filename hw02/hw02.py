...
def accumulate(combiner, base, n, term):
    ...
    def repeat(i):
        if i == 1:
            return term(1)
        return combiner(repeat(i-1), term(i))
    return combiner(base, repeat(n))
...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(element, index, added):
        if index == n:
            return element
        if index % 7 == 0 or num_sevens(index) != 0:
            return helper(element + (-added), index + 1, -added)
        return helper(element + added, index + 1, added)
    return helper(1, 1, 1)
...
def count_change(amount):
    ...
    """
    "*** YOUR CODE HERE ***"
    def max_denomination_from(denomination):
        if denomination > amount:
            return denomination // 2
        return max_denomination_from(denomination * 2)
    ...
    return count_partition(amount, max_denomination_from(1))
