...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    return (length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m)))) and (is_weight(end(left(m))) or balanced(end(left(m))) ) and (is_weight(end(right(m))) or balanced(end(right(m))))
